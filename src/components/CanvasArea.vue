  ...
  <div class="canvas">
    <drop @drop="handleDrop">
      <div
        ref="canvasarea"
        class="canvasarea"
        :style="{background: `url('${selectedBackgroundUrl}')`}"
      >
...
export default {
    ...
    if (this.$route.params.id !== undefined) {
      ...
      if (parsedData && parsedData[this.$route.params.id]) {
        ...
        this.selectedBackgroundUrl =
          parsedData[this.$route.params.id].background;
      }
    }
  ...
  methods: {
    ...
    rotateLeft() {
      ...
      if (this.selected) {
        ...
        if (rotateString) {
          ...
          this.cutsOnCanvas[idx].transform = this.cutsOnCanvas[
            idx
          ].transform.replace(
            `rotate(${rotateString[1]}deg)`,
            `rotate(${rotateData - 30}deg)`
          );
        } else {
          this.cutsOnCanvas[idx].transform =
            this.cutsOnCanvas[idx].transform + " rotate(-30deg)";
        }
      }
    },
    rotateRight() {
    ...
    handleDrag({ target, transform }) {
      ...
      const idxCut = this.cutsOnCanvas.findIndex(
        item => item.id === this.selected
      );
    ...
    handleDrop(data, event) {
      this.cutsOnCanvas.push({
        ...data,
        orderIndex: 1073741823 /* +this.cutsOnCanvas.length */,
        id: uuidv4(),
        transform: `translate(${event.clientX -
          data.offsetX -
          100}px, ${event.clientY - data.offsetY - 100}px)`
      });
    }
  }
};
